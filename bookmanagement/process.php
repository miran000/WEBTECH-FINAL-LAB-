    ...
    !empty($_POST['returnDate'])) {
        ...
        isset($_POST['returnDate']))
        {
            ...
            if(preg_match("/^[0-9]*$/", $_POST['token'])){
                ...
                foreach($json[0]->token as $jsonToken){
                    if($Token == $jsonToken){
            ...
            if(($interval->days > 10) && $tokenValid==false){
                ...
                if(isset($_COOKIE[$cookie_name])){
                    ...
                    //Set Cookies
                    $usedTokenJson = json_decode(file_get_contents('usedTokens.json'));
                    $usedTokenJson[0]->usedTokens[] = $token;
                    $updatedJsonData = json_encode($usedTokenJson, JSON_PRETTY_PRINT);
                    file_put_contents('usedTokens.json', $updatedJsonData);
                    setcookie($cookie_name, $cookie_value, time() + 35);
                    setcookie($token_name, $cookie_value, time() + 35);
